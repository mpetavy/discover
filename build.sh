...
# to run this image: docker run --env discover_limit_discover_port=9999 --name discover-app discover-app
...
if [ -z "$TEAMCITY_VERSION" ]; then
  ...
  docker cp $APP:/go/src/$APP/dist-linux-amd64/discover .
  ...
  # docker tag discover-app mpetavy/discover-app
  # docker commit -m "discover-app" -a mpetavy `docker container ls -lq` mpetavy/discover-app
  # docker push mpetavy/discover-app
fi
